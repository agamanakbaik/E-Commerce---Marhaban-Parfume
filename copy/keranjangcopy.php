...
<body class="gradient-bg min-h-screen">
    ...
    <nav class="bg-white shadow-lg">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center">
                    <a href="#"
                        class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-[#099ea3]">Home</a>
                    <a href="#"
                        class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-[#099ea3]">Products</a>
                    <a href="#"
                        class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-[#099ea3]">About</a>
                    <div class="ml-4 relative">
                        <button class="text-gray-700 hover:text-[#099ea3]">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div class="flex flex-col items-center">
                    <div class="w-10 h-10 rounded-full bg-[#099ea3] text-white flex items-center justify-center">
                        <i class="fas fa-shopping-cart"></i>
                    </div>
                    <span class="mt-2 text-sm font-medium text-[#099ea3]text-purple-600">Cart</span>
                </div>
                <div class="flex-1 h-1 mx-2 bg-[#099ea3]"></div>
                <div class="flex flex-col items-center">
                    <div class="w-10 h-10 rounded-full bg-[#099ea3] text-white flex items-center justify-center">
                        <i class="fas fa-truck"></i>
                    </div>
                    <span class="mt-2 text-sm font-medium text-[#099ea3]">Shipping</span>
                </div>
                <div class="flex-1 h-1 mx-2 bg-[#099ea3]"></div>
                <div class="flex flex-col items-center">
    ...
    <!-- Item akan ditampilkan di sini oleh JavaScript -->
</div>
...
async function loadCart() {
    try {
        // 1. Ambil dari server (untuk sinkronisasi)
        const response = await fetch('api/cart.php');
        const serverCart = await response.json();
        
        // 2. Sinkronkan dengan localStorage
        if (serverCart.success) {
            localStorage.setItem('cart', JSON.stringify(serverCart.data));
            renderCart(serverCart.data);
        } else {
            throw new Error(serverCart.message || 'Gagal memuat keranjang');
        }
    } catch (error) {
...
function renderCart(cart) {
    ...
    if (cart.length === 0) {
        cartContainer.innerHTML = '<p class="text-gray-500">Keranjang Anda kosong</p>';
        return;
    }
    
    cartContainer.innerHTML = cart.map(item => `
        <div class="flex items-center justify-between border-b pb-4" id="item-${item.id}">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-sm font-medium text-gray-800">Rp ${item.price.toLocaleString()}/item</p>
                </div>
            </div>
            <div class="flex items-center">
                <button class="text-gray-500 hover:text-purple-600 px-2" onclick="updateQuantity('${item.id}', -1)">
                    <i class="fas fa-minus"></i>
                </button>
                <span id="${item.id}-qty" class="mx-2 text-gray-700">${item.quantity}</span>
                <button class="text-gray-500 hover:text-purple-600 px-2" onclick="updateQuantity('${item.id}', 1)">
                    <i class="fas fa-plus"></i>
                </button>
                <span id="${item.id}-price" class="ml-6 font-medium text-gray-800">Rp ${(item.price * item.quantity).toLocaleString()}</span>
                <button class="ml-4 text-red-500 hover:text-red-700" onclick="removeItem('${item.id}')">
...
async function updateQuantity(id, change) {
    try {
        ...
        if (result.success) {
            ...
            document.getElementById(`${id}-qty`).textContent = item.quantity;
            document.getElementById(`${id}-price`).textContent = `Rp ${(item.price * item.quantity).toLocaleString()}`;
            updateTotal();
        } else {
            throw new Error(result.message || 'Gagal memperbarui jumlah');
        }
    } catch (error) {
        console.error('Error:', error);
...
async function removeItem(id) {
    try {
        ...
        if (result.success) {
            ...
            if (cart.length === 0) {
                document.getElementById('cart-items').innerHTML = '<p class="text-gray-500">Keranjang Anda kosong</p>';
            }
        } else {
